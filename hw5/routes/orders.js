...
let MonthData = class {
  ...
  const janData = new MonthData(0, 0, 0);
  const febData = new MonthData(0, 0, 0);
  const marData = new MonthData(0, 0, 0);
  const aprData = new MonthData(0, 0, 0);
  const mayData = new MonthData(0, 0, 0);
  const junData = new MonthData(0, 0, 0);
  const julData = new MonthData(0, 0, 0);
  const augData = new MonthData(0, 0, 0);
  const sepData = new MonthData(0, 0, 0);
  const octData = new MonthData(0, 0, 0);
  const novData = new MonthData(0, 0, 0);
  const decData = new MonthData(0, 0, 0);

// get query
dbquery("select Topping, Month, count(*) from ORDERS GROUP BY Month, Topping", storeResults);
...
router.post('/', function(req, res, next) {
    var toReturn = dispay(req.body.Month); // call helper function to get JSON object
    res.json(toReturn);
});
